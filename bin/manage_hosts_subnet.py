...
AVAILABLE_HOSTS = dict(
    {'10.0.7': {
         ...
         'lnls350-linux': '00:25:90:E7:58:8B',
     }, '10.0.21': {
...
def check_hosts():
    ...
    hosts_online = {result[x]['addresses'].get('mac', my_mac): x
    ...
    for name in sorted(hosts_in_my_subnet):
        ...
        if mac in hosts_online:
            print(STRFORMAT.format(name, 'up',
                                   hosts_online[mac].replace(':', '.')))
        else:
...
def control_service(hosts, wt2do='start'):
    ...
    COMM = 'net rpc service {:s} VBoxVmService -S {:s} -U ABTLUS\\\{:s}%{:s}'
    usuario = input('User on remote host: ')
    senha = getpass.getpass('Password on remote host: ')

    for host in hosts:
        print(host, ':')
        out = subproc.getoutput(COMM.format(wt2do, host, usuario, senha))
        # out = COMM.format(wt2do, host, usuario, senha)
        print(out)
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '-b', '--startVBox', dest='start', type='str',
        help="Start VBoxVmService hosts. [host1,...,hostN]")
    parser.add_option(
        '-s', '--statusVBox', dest='status', type='str',
        help="Verify status of VBoxVmService on hosts. [host1,...,hostN]")
    parser.add_option(
        '-p', '--stopVBox', dest='stop', type='str',
        help="Stop VBoxVmService hosts. [host1,...,hostN]")
